...
abstract class BaseTable
{
     ...
     * @throws Exception
     */
    public function getDb($dbkey = '')
    {
        $dbConnection = $this->getDbsession()->getDbConnection($dbkey ? $dbkey : $this->dbKey());
        if (is_null($dbConnection)) {
            throw new Exception($this->getDbsession()->formatMessage('db_non_existent',['dbkey'=>$dbkey]));
        }
     ...
     * @return $this
     */
    public function setClass($stdClass = true)
    {
     ...
     * @return $this
     */
    public function setScope($scope,...$args)
    {
        if (empty($scope)) {
    ...
    public function setTable($table = '')
    {
        ...
        if (is_array($table)) {
            ...
            $table = $this->splitTableName($table);
            list($table,$alias) = $table;
        }
     ...
     * 1、一般用查询时，设置读取的字段列
     * 2、连贯操作
     * 3、如果参数$fields 为空，则读取所有字段列
     *</pre>
     *<B>示例：</B>
     *<pre>
     *    示例1：查询数据-字符串字段列
     *    $where = array('name'=>'admin');
     *    $this->setTable('User')->setSelect('id,username,pwd')->setWhere($where)->queryRows();
     *
     *    示例2：查询数据-数组字段列
     *    $where = array('name'=>'admin');
     *    $this->setTable('User')->setSelect(['id','username','pwd'])->setWhere($where)->queryRows();
     *
     *        示例3：查询数据-定义别名字段列，
     *        $this->setTable('User')->setSelect(['id'=>'_id','username'=>'name','password'=>'pwd'])->setWhere($where)->queryRows();
     *
     *        示例4：查询数据-定义字段方法，别名, id 最小值
     *        $this->setTable('User')->setSelect(['id'=>['alias'=>'_id','method'=>'min']])->setWhere($where)->queryRows();
     *</pre>
     ...
     *<pre>
     * string: 'u' 单表别名，多表名称:'tb1,tb2,tb3',
     * array:array('表名1'=>'别名1','表名2'=>'别名2');
     *</pre>
     * @return $this
     */
     ...
     *    1、设置查询条件、更新条件
     *    2、连贯操作
     *        3、sql条件 支持三种格式:字符串方式，hash（哈希）方式,混合 字符串+hash，对应hash,混合方式的条件，数组的每一个元素 都可以是嵌套的hash,或混合条件
     *            字符串:'userid=2'
     *          基本格式['字段'] = ['操作符','数值'];
     *            hash:即关联数组,['username'=>'admin','userid'=>['in','1,2,3']]
     *            混合:['userid'=>['in','1,2,3'],'userid=2']
     *        4、支持多次调用
     *        5、where 条件不支持全数字作为字段名
     *        6、where 条件的解析使用了递归方式
     *</pre>
     *<B>示例：</B>
     *<pre>
     *    示例1：字符串条件 不推荐使用此种方式
     ...
     * @return $this
     */
    public function setWhere($where = [], $params = [],$op = null)
    {
        // 闭包方式
        ...
        $field = $this->formatColumn($where);
        $_where = [];
        $_where[$field] = [$op, $params];
        $this->setAndWhere($_where);
     ...
     * @return $this
     */
    public function setAndWhere($where = [], $params = [])
    {
     ...
     * @return $this
     */
    public function setOrWhere($where = [], $params = [])
    {
        if (isset($this->options['where']) && !empty($this->options['where'])) {
     ...
     * @return $this
     */
    public function setJoin($table, $on, $joinType = '')
    {
     ...
     * @return $this
     */
    public function setWith($with,$join = false,$load = true)
    {
        if (is_null($with)) {
            $this->options['with'] = [];
            return $this;
    ...
    public function setLeftWith($with,$load = true)
    {
    ...
    public function setInnerWith($with,$load = true)
    {
     ...
     *  示例1：sql 查询
     *</pre>
     * @param  string|Query $union sql指令
     * @param  boolean $all 绑定参数
     * @return $this
     */
    public function setUnion($union, $all = false)
    {
        if ($all === true) {
     ...
     * @return $this
     */
    public function setLimit($length = null)
    {
     ...
     * @return $this
     */
    public function setAndHaving($having = [], $params = [])
    {
        if (isset($this->options['having'])) {
     ...
     * @return $this
     */
    public function setOrHaving($having = [], $params = [])
    {
        if (isset($this->options['having'])) {
     ...
     * @return $this
     */
    public function setShard($shard_columns = [])
    {
     ...
     *  示例1：
     *    $data = ['username'=>'121'];
     *  $this->setReplace()->setData($data)->addRow();
     *</pre>
     * @param boolean $replace 是否启用
     * @return $this
     */
    ...
    public function asMaster($asMaster = true)
    {
    ...
    public function queryRow($options = null)
    {
        ...
        $queryResult = $this->buildWith($query,$queryResult);
        unset($query);
     ...
     *</pre>
     */
    public function queryCmd($queryCommand, $params = [])
    {
        $query = $this->getQuery();
     ...
     *</pre>
     */
    abstract public function addInternal($query, $queryType = '');
     ...
     *</pre>
     */
    abstract public function updateInternal($query, $queryType = '');
     ...
     *</pre>
     */
    abstract public function deleteInternal($query, $queryType = '');
     ...
     *</pre>
     */
    abstract public function queryInternal($query, $queryType = '');
     ...
     *</pre>
     */
    abstract public function queryScalarInternal($query, $method = '');
     ...
     *</pre>
     */
    protected function queryCommand($query)
    {
        $command = $query->buildParamsCommand();
    ...
    {
        return $this->getDbsession()->getLastId();
    }
     ...
     * @param Query $query
     */
    protected function parseWith($query)
    {
        $withs = $query->getWith();
        if (empty($withs)) {
            return ;
        ...
        foreach ($withs as $name=>$item) {
            ...
            list($name,$ref_alias)  = $this->splitTableName($name);
            ...
            $join = [[$ref_query->getTableName(),$ref_alias],$refs,$joinType];
            $query->addJoin($join);
     ...
     * @return Query
     */
    protected function buildWith($query,$result)
    {
        if (empty($result)) {
            return $result;
        }
        ...
        foreach ($withs as $name=>$item) {
            list($name, $load,$join, $hander) = $item;
            list($name, $alias) = $this->splitTableName($name);
            ...
            foreach ($result as $index=>$res) {
                $pk_id = $res[$ref_key];
                $res[$name] = isset($ref_entity_dict[$pk_id]) ? $ref_entity_dict[$pk_id]:null;
                $result[$index] = $res;
            }
        }
    ...
    public function formatColumn($column_name)
    {
        if (substr($column_name,0,1) == '#') {
            $table_alias = $this->getAlias();
            if (!empty($table_alias)) {
                return str_replace('#',$table_alias.'.',$column_name);
            }
        }
